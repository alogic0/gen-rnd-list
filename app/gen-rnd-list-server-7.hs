...
convert seed content = map unlines
  . map (concatMap (wrap maxLine)) . groupN 6     -- number of prefixes in transform
  . transform seed
...
transform seed ls = 
  let sumN = sum . snd . unzip $ ls
      ...
      fChop ((client, n) : clients) bigLst =
         client
         : preLine
         : replicate (length preLine) '*'
         : (localShowList widthN . sort . take n $ bigLst)
         : replicate (length postLine) '*' 
         : postLine
         : fChop clients (drop n bigLst)
        where
          preLine = "Всего бонусов: " ++ show n ++ " из " ++ show sumN
          postLine = "Всего бонусов: " ++ show n
      fChop _ [] = []
      fChop [] _ = []
  in
